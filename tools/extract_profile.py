...
def calculate_enhanced_completeness(profile_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    enhanced_factors = {
        "has_comprehensive_about": bool(profile_data.get("about", "").strip() and len(profile_data.get("about", "")) > 100),
        "has_multiple_experiences": len(profile_data.get("experiences", [])) >= 2,
        "has_detailed_experiences": any(
            exp.get("description", "").strip() for exp in profile_data.get("experiences", [])
        ),
        "has_education_details": any(
            edu.get("degree", "").strip() for edu in profile_data.get("education", [])
        ),
    ...
    completeness_data = {
        ...
        "factors": {
            "basic": {
                "has_name": bool(profile_data.get("name", "").strip()),
                "has_headline": bool(profile_data.get("headline", "").strip()),
                "has_about": bool(profile_data.get("about", "").strip()),
