...
class RolePromptBuilder:
    ...
    def build_enhanced_prompt(
        ...
        prompt_parts.extend([
            ...
            f"• CRITICAL: Store strategy_data['strategy'] in memory - you'll need it for ALL subsequent phases",
            ...
            f"• CRITICAL: Use strategy from Step 1: search_results = smart_candidate_search(query='{base_query}', location='{location}', target_candidate_count={limit}, strategy=strategy_data['strategy'])",
            f"• Note: Strategy is required for accurate candidate quality assessment during search",
            f"• CHECK result: if 'extraction_quality_gate' failed → retry search with adjusted params",
            f"• if search successful → GO TO EXTRACT PHASE",
            f"",
            f"EXTRACT PHASE:",
            ...
            f"  → 'try_heap_backups': backup_candidates = get_collected_candidates() → extract_candidate_profiles() → evaluate with backup_data['results']",
            f"  → 'expand_search_scope': smart_candidate_search(start_page=X) → EXTRACT PHASE → evaluate with new extracted_data['results']", 
            f"  → 'lower_quality_threshold': evaluate_candidates_quality(candidates=extracted_data['results'], strategy=strategy_data['strategy'], threshold=4.0) → check result (LAST RESORT)",
            f"",
            f"OUTREACH PHASE (Final):",
            f"• generate_candidate_outreach(candidates=eval_results['quality_candidates'], position_title='{base_query}', location='{location}')",
            ...
            f"- Step 2: smart_candidate_search(strategy=strategy_data['strategy']) → search_results",
            ...
            f"  • try_heap_backups: get_collected_candidates → EXTRACT PHASE → EVALUATE PHASE (use same strategy)",
