...
class RolePromptBuilder:
    ...
    def build_enhanced_prompt(
        ...
        prompt_parts.extend([
            ...
            f"• REMEMBER: extracted_data['results'] = list of ALL candidates for next phase",
            f"• Always proceed to → EVALUATE PHASE with ALL extracted_data['results']",
            f"",
            ...
            f"• AUTOMATICALLY CLEAN HEAP: Low-scoring candidates are removed from the heap during evaluation",
            ...
            f"  → 'try_heap_backups': Access remaining quality candidates from cleaned heap (PRIORITY 1)",
            f"    Use backup_offset from eval_results['fallback_params']['backup_offset'] (tracks processed count)",
            f"    backup_result = smart_candidate_search(get_heap_backup=True, **eval_results['fallback_params'])",
            f"    → MUST extract_candidate_profiles() → MUST evaluate_candidates_quality(candidates=extracted_data['results'], strategy=strategy_data['strategy'])",
            f"    CRITICAL: Complete full EXTRACT → EVALUATE cycle for heap backup candidates",
            f"    REASON: Systematically exhaust heap before new search (no duplication, faster)",
            f"  → 'expand_search_scope': Search new pages from where last search stopped (PRIORITY 2)",
            f"    Use start_page from eval_results['fallback_params']['start_page'] (continues pagination)",  
            f"    search_result = smart_candidate_search(**eval_results['fallback_params'], strategy=strategy_data['strategy'])",
            f"    → MUST extract_candidate_profiles() → MUST evaluate_candidates_quality(candidates=extracted_data['results'], strategy=strategy_data['strategy'])",
            f"    CRITICAL: Complete full EXTRACT → EVALUATE cycle for new search candidates",
            f"    REASON: Find fresh candidates when heap is fully exhausted",
            f"  → 'lower_quality_threshold': Lower standards as last resort (PRIORITY 3)",
            f"    evaluate_candidates_quality(candidates=extracted_data['results'], strategy=strategy_data['strategy'], min_quality_threshold=4.0)",
            f"    REASON: Accept lower quality candidates when search space is exhausted",
            f"",
            f"IMPROVED HEAP MANAGEMENT STRATEGY:",
            f"• During evaluation, bad candidates (score < threshold) are automatically removed from heap",
            f"• This keeps the heap clean with only quality candidates for systematic exhaustion",
            f"• backup_offset = total_evaluated ensures no candidate duplication (tracks processed count)",
            f"• expand_search_scope uses stored next_start_page to continue from where last search stopped",
            f"• NO REDUNDANT PROCESSING: heap exhaustion → page continuation → no overlaps",
            f"• Heap size monitoring: only expand search when remaining heap < target candidate count",
            f"",
