...
class KeySignals(BaseModel):
    ...
    role_match: bool = Field(
        description="Whether the candidate's role matches the target role"
    )
    seniority_appropriate: bool = Field(
        description="Whether the seniority level is appropriate"
    )
    tech_stack_match: float = Field(
        description="Technology stack alignment score (0.0-1.0)"
    )
    company_tier: str = Field(
        description="Company tier classification (tier_1, tier_2, tier_3)"
    )
...
class CandidateAssessment(BaseModel):
    ...
    overall_score: float = Field(
        description="Overall candidate quality score (0.0-10.0)"
    )
    technical_score: float = Field(
        description="Technical skills and expertise score (0.0-10.0)"
    )
    experience_score: float = Field(
        description="Experience level and career progression score (0.0-10.0)"
    )
    cultural_fit_score: float = Field(
        description="Likely cultural and team fit score (0.0-10.0)"
    )
    extraction_quality: float = Field(
        description="Quality of available profile data (0.0-10.0)"
    )
    profile_completeness: float = Field(
        description="Profile data completeness percentage (0.0-100.0)"
    )

    strengths: List[str] = Field(
        description="List of candidate strengths and positive indicators"
    )
...
class SearchBudget:
    ...
    min_heap_capacity_pct: float = (
        50.0  # Minimum heap capacity utilization before extraction
    )
...
class CandidateHeap:
    ...
    def add_candidate(
        self, candidate_data: Dict[str, Any], quality: CandidateAssessment
    ):
        ...
        if len(self.heap) < self.max_size:
            heapq.heappush(
                self.heap, (quality.overall_score, url, candidate_data, quality)
            )
    ...
    def get_top_candidates(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        return [
            {**candidate_data, "quality_assessment": quality, "heap_rank": i + 1}
    ...
    def needs_more_candidates(self, thresholds: QualityThresholds) -> bool:
        ...
        acceptable_count = sum(
            1 for score, _, _, _ in self.heap if score >= thresholds.minimum_acceptable
        )
    ...
    def remove_candidate(self, url: str) -> bool:
        ...
        for score, candidate_url, candidate_data, quality in original_heap:
            if candidate_url != url:
                heapq.heappush(
                    self.heap, (score, candidate_url, candidate_data, quality)
                )
    ...
    def should_continue_search_for_capacity(
        self, min_capacity_pct: float = 50.0
    ) -> bool:
    ...
    def is_ready_for_extraction(
        self,
        min_avg_threshold: float = 5.0,
        min_candidates: int = 3,
        min_capacity_pct: float = 50.0,
    ) -> tuple:
        ...
        return (
            True,
            f"ready_quality_{avg_quality:.1f}_capacity_{capacity_utilization:.1f}pct",
        )
...
class QualityGenerator:
    ...
    def __init__(self):
        ...
        self.generator = Generator(
            ...
            prompt_kwargs={"output_format_str": self.parser.format_instructions()},
            output_processors=self.parser,
        )

    def assess_candidate_quality_llm(
        self,
    ...
    def _create_fallback_quality(
        self, candidate_data: Dict[str, Any], role_query: str
    ) -> CandidateAssessment:
        ...
        return CandidateAssessment(
            ...
            key_signals=KeySignals(
                ...
                seniority_appropriate=any(
                    word in headline for word in ["senior", "lead", "principal"]
                ),
                tech_stack_match=0.5,  # Default middle value for fallback
                company_tier="unknown",
            ),
            extraction_quality=7.0
            if candidate_data.get("name") and candidate_data.get("headline")
            else 3.0,
...
class QualityAnalyzer:
    ...
    def should_extend_search(
        self, current_stats: Dict[str, float], pages_searched: int
    ) -> tuple:
