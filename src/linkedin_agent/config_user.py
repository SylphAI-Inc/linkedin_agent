...
class SearchConfig:
    ...
    # Search Parameters
    # Network connection filter, "F" for 1st degree (default), "S" for 2nd, "O" for 3rd+, "" for all
    network_filter: str = ""
...
class UserConfig:
    ...
    def load_from_env(cls) -> "UserConfig":
        ...
        if os.getenv("MIN_EVALUATION_THRESHOLD"):
            config.search.min_evaluation_threshold = float(
                os.getenv("MIN_EVALUATION_THRESHOLD")
            )
        if os.getenv("TARGET_QUALITY_CANDIDATES"):
            config.search.target_quality_candidates = int(
                os.getenv("TARGET_QUALITY_CANDIDATES")
            )
        ...
        if os.getenv("EXTRACTION_DELAY"):
            config.extraction.delay_between_extractions = float(
                os.getenv("EXTRACTION_DELAY")
            )
