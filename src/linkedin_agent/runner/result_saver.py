...
def save_recruitment_results(
    candidates: List[Dict[str, Any]],
    search_params: Dict[str, Any],
    output_dir: str = "results",
    strategy_data: Dict[str, Any] | None = None,
    outreach_data: Dict[str, Any] | None = None,
) -> Dict[str, str]:
    ...
    repo_root = current_file.parents[
        3
    ]  # Go up from src/linkedin_agent/runner/ to repo root
    ...
    evaluation_summary = {
        ...
        "candidate_evaluations": _extract_candidate_evaluations(
            candidates, strategy_data
        ),
    }
    ...
    with open(txt_file, "w", encoding="utf-8") as f:
        f.write("LinkedIn Recruitment Results\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"Search Query: {search_params['query']}\n")
        f.write(f"Location: {search_params['location']}\n")
        f.write(
            f"Enhanced Prompting: {'Yes' if search_params.get('enhanced_prompting') else 'No'}\n"
        )
        f.write(
            f"Strategy Used: {'Yes' if search_params.get('strategy_used') else 'No'}\n"
        )
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            f.write(
                f"Name: {_safe_get(profile_info, 'name', _safe_get(search_info, 'name', 'N/A'))}\n"
            )
            f.write(
                f"Title: {_safe_get(profile_info, 'headline', _safe_get(search_info, 'subtitle', 'N/A'))}\n"
            )
            ...
            if (
                ...
                for exp in experiences[:2]:  # Show first 2 experiences
                    if isinstance(exp, dict):
                        f.write(
                            f"  • {_safe_get(exp, 'title', 'N/A')} at {_safe_get(exp, 'company', 'N/A')}\n"
                        )
            ...
            if overall_score > 0:
                ...
                if isinstance(quality, dict) and quality.get("completeness_percentage"):
                    f.write(
                        f"Profile Completeness: {quality['completeness_percentage']}%\n"
                    )
...
def _extract_candidate_evaluations(
    candidates: List[Dict[str, Any]], strategy_data: Dict[str, Any] | None = None
) -> List[Dict[str, Any]]:
    ...
    for candidate in candidates:
        ...
        evaluation_data = {
            ...
            "headline_score": candidate.get("search_info", {}).get(
                "headline_score", 0.0
            ),
        }
            ...
            evaluation_data.update(
                {
                    ...
                    "strategic_bonuses": _safe_get_dict(
                        quality, "strategic_bonuses", {}
                    ),
                    "scoring_weights": _safe_get_dict(quality, "scoring_weights", {}),
                    "final_multiplier": _safe_get_dict(
                        quality, "final_multiplier", 1.0
                    ),
                    "meets_threshold": _safe_get_dict(
                        quality, "meets_threshold", False
                    ),
                    "strengths": _safe_get_dict(quality, "strengths", []),
                    "concerns": _safe_get_dict(quality, "concerns", []),
                    "evaluation_timestamp": _safe_get_dict(
                        quality, "evaluation_timestamp", ""
                    ),
        ...
        if outreach_info:
            evaluation_data["outreach_data"] = {
                ...
                "personalized_message_available": bool(
                    outreach_info.get("message", "")
                ),
            }
...
def _extract_candidate_details(
    ...
    for candidate in candidates:
        ...
        detail_data = {
            ...
            "extraction_metadata": {
                "extraction_quality": profile_details.get("quality_assessment", {}).get(
                    "extraction_quality", 0.0
                ),
                "profile_completeness": profile_details.get(
                    "quality_assessment", {}
                ).get("profile_completeness", 0.0),
                "timestamp": profile_details.get("quality_assessment", {}).get(
                    "timestamp"
                ),
            },
        }
