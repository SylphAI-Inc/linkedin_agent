...
def setup_chrome_cdp():
    ...
    if _cdp_alive():
        log_info(
            f"Attaching to existing Chrome on port {cdp.port}", phase="initialization"
        )
    else:
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="LinkedIn Recruitment Agent - Clean Architecture"
    )
...
def main():
    ...
    if not args.query and not args.job_description:
        log_error(
            "Either --query or --job-description must be provided",
            phase="initialization",
        )
        log_info("Examples:", phase="initialization")
        log_info(
            "   python runner/run_linkedin_agent_clean.py --query 'Senior Engineer' --location 'SF'",
            phase="initialization",
        )
        log_info(
            "   python runner/run_linkedin_agent_clean.py --job-description my_job.txt",
            phase="initialization",
        )
        return
    ...
    if args.job_description:
        try:
            ...
            if not job_file.exists():
                log_error(
                    f"Job description file not found: {args.job_description}",
                    phase="initialization",
                )
                sys.exit(1)
            ...
            log_info(
                f"Loaded job description from: {args.job_description} ({len(job_description)} characters)",
                phase="initialization",
            )
        except Exception as e:
            log_error(
                "Error reading job description file",
                phase="initialization",
                exception=e,
            )
            sys.exit(1)
    ...
    log_info(
        "   4. EVALUATE → Score candidates with strategic bonuses", phase="workflow"
    )
    ...
    if candidates:
        ...
        if "Error:" not in str(save_info.get("evaluation_file", "")):
            ...
            log_info(
                f"Evaluation File: {save_info['evaluation_file']}", phase="analysis"
            )
            log_info(
                f"Candidates File: {save_info['candidates_file']}", phase="storage"
            )
        ...
        log_debug("Strategy: AI-Generated by Agent", phase="debug")
        ...
        for i, candidate in enumerate(candidates[:3], 1):
            try:
                ...
                log_debug(
                    f"{i}. [Error displaying candidate: {display_error}]", phase="debug"
                )

    else:
        log_error(" No candidates found", phase="error")
        log_info("💡 Try adjusting your search criteria or location")
        log_info("💡 Check your LinkedIn authentication or network connection")

    # Log final summary
    duration = (
        time.time() - workflow_start_time if "workflow_start_time" in locals() else 0
    )
    logger.workflow_summary(len(candidates), duration, len(candidates) > 0)

    log_phase_end(
        "MAIN", len(candidates) > 0, f"Processed {len(candidates)} candidates"
    )

    log_info("✅ Recruitment workflow completed!")
    if candidates:
        log_info("📊 Final result: Successfully processed {len(candidates)} candidates")
    else:
        log_info(
            "📊 Final result: No candidates found - consider adjusting search parameters"
        )
