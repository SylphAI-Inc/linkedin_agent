...
def generate_candidate_outreach(
    ...
    log_info(
        f"🔄 Retrieved {len(candidates)} quality candidates from global state",
        phase="OUTREACH",
    )
    ...
    log_info(
        f"📊 Generating outreach for {len(candidates)} candidates...", phase="OUTREACH"
    )
    ...
    for i, candidate in enumerate(candidates, 1):
        log_progress(
            f"Generating outreach for {candidate.get('name', 'Unknown')} ({i}/{len(candidates)})",
            "OUTREACH",
        )

        try:
            # Generate message directly without evaluation (candidates are pre-qualified)
            outreach_result = _generate_message_for_candidate(
                candidate, position_title, location
            )
    ...
    print(
        f"💾 {global_state_result.get('message', 'Outreach results stored in global state')}"
    )
    ...
    return {
        ...
        "outreach_generated": len(outreach_results),
        "recommended_count": successful_outreach,
        "success_rate": (successful_outreach / len(candidates) * 100)
        if len(candidates) > 0
        else 0,
...
def _extract_candidate_name(candidate: Dict[str, Any]) -> str:
    ...
    if "candidate_info" in candidate and "profile_data" in candidate:
        return candidate["profile_data"].get(
            "name", candidate["candidate_info"].get("name", "Unknown")
        )
    return candidate.get("name", "Unknown")


def _generate_message_for_candidate(
    candidate: Dict[str, Any], position_title: str, location: str
) -> Dict[str, Any]:
    ...
    if "candidate_info" in candidate and "profile_data" in candidate:
        ...
        if overall_score == 0.0:
            ...
            overall_score = profile_data.get("overall_score", 0.0) or profile_data.get(
                "quality_assessment", {}
            ).get("overall_score", 0.0)

        # If still not found, try candidate_info locations
        if overall_score == 0.0:
            overall_score = candidate_info.get(
                "overall_score", 0.0
            ) or candidate_info.get("quality_assessment", {}).get("overall_score", 0.0)
        ...
        if overall_score == 0.0:
            overall_score = candidate.get("quality_assessment", {}).get(
                "overall_score", 0.0
            )
        ...
        if overall_score == 0.0:
            ...
            print(
                f"      ⚠️ WARNING: Could not find overall_score for {candidate_name} (flat structure)"
            )
            print(f"         Available keys: {list(candidate.keys())}")
        else:
            candidate_name = candidate.get("name", "Unknown")
            print(
                f"      ✅ Found overall_score: {overall_score} for {candidate_name} (flat structure)"
            )
    ...
    try:
        ...
        outreach_result = manager.evaluate_candidate_for_outreach(candidate_profile, {})

        # Extract message from LLM result
        if outreach_result.get("recommend_outreach") and outreach_result.get(
...
def _create_enhanced_outreach_message(
    candidate_profile: Dict[str, Any], position_context: Dict[str, Any]
) -> str:
    ...
    position_title = (
        position_context.get("title", "Software Engineer")
        if position_context
...
def _build_personalization_content(
    name: str,
    headline: str,
    ...
    if experiences and len(experiences) > 0:
        ...
        current_company = (
            company if company else _extract_company_from_headline(headline)
        )
    ...
    if relevant_skills:
        connection_parts.append(
            f"Your expertise in {', '.join(relevant_skills[:3])} aligns perfectly with what we're looking for."
        )

    if current_company and current_company not in [
        "Unknown",
        "",
        ...
        connection_parts.append(
            f"Your experience at {current_company} demonstrates the high-caliber background we seek."
        )
    ...
    if not connection_parts:
        connection_parts.append(
            f"Your {years_experience} professional experience in software development makes you an ideal candidate."
        )
...
def _generate_single_outreach(
    candidate: Dict[str, Any], position_context: str, min_score: float
) -> Dict[str, Any]:
...
class CandidateOutreachManager:
    ...
    def evaluate_candidate_for_outreach(
        ...
        try:
            ...
            evaluation_result = self.evaluator(prompt_kwargs=prompt_data)
            ...
            if hasattr(evaluation_result, "data") and evaluation_result.data:
                ...
                print(
                    f"⚠️  DataClass parsing failed, raw response available: {type(evaluation_result.raw_response)}"
                )
                evaluation_obj = evaluation_result
            else:
                print(
                    f"⚠️  Unexpected evaluation result format: {type(evaluation_result)}"
                )
                evaluation_obj = evaluation_result
            ...
            if isinstance(evaluation_obj, CandidateOutreachEvaluation):
                ...
                if not evaluation_obj.candidate_name:
                    evaluation_obj.candidate_name = candidate_profile.get(
                        "name", "Unknown"
                    )

                # Apply strategic bonuses if strategy provided
                if strategy:
                    evaluation_obj = self._apply_strategic_bonuses(
                        evaluation_obj, candidate_profile, strategy
                    )
                ...
                try:
                    ...
                    raw_response = (
                        evaluation_result.raw_response
                        if hasattr(evaluation_result, "raw_response")
                        else None
                    )
                    if (
                        raw_response
                        and hasattr(raw_response, "output")
                        and raw_response.output
                    ):
                        ...
                        if hasattr(output_msg, "content") and output_msg.content:
                            ...
                            if json_match:
                                ...
                                evaluation_obj = CandidateOutreachEvaluation.from_dict(
                                    evaluation_dict
                                )
                                evaluation_data = evaluation_obj.to_dict()
                                # Apply strategic bonuses if strategy provided
                                ...
                                evaluation_data.update(
                                    {
                                        "candidate_url": candidate_profile.get(
                                            "url", ""
                                        ),
                                        "evaluation_timestamp": datetime.now().isoformat(),
                                        "evaluator_version": "v1.0",
...
"""
        ...
        if education:
            ...
            for edu in education[:2]:  # Show top 2 education entries
                formatted += (
                    f"- {edu.get('degree', 'N/A')} from {edu.get('school', 'N/A')}\n"
                )
    ...
    def _apply_strategic_bonuses(
        self,
        evaluation_obj: CandidateOutreachEvaluation,
        candidate_profile: Dict[str, Any],
        strategy: Dict[str, Any],
    ) -> CandidateOutreachEvaluation:
        ...
        headline = candidate_profile.get("headline", "") or profile_data.get(
            "headline", ""
        )
        ...
        if total_bonus > 0:
            print(
                f"      📈 Strategic bonuses: Company(+{company_bonus:.1f}) + Seniority(+{seniority_bonus:.1f}) + Tech(+{tech_stack_bonus:.1f}) = +{total_bonus:.1f}"
            )
        ...
        if new_score >= 35.0 and not evaluation_obj.recommend_outreach:
            ...
            print(
                f"      🎯 Strategic bonus triggered outreach recommendation: {original_score:.1f} → {new_score:.1f}"
            )
    ...
    def _calculate_company_bonus(
        self, analysis_text: str, strategy: Dict[str, Any]
    ) -> float:
        ...
        if "target_companies" in strategy:
            ...
            "headline_analysis" in strategy
            and "company_indicators" in strategy["headline_analysis"]
        ):
    ...
    def _calculate_seniority_bonus(
        self, analysis_text: str, strategy: Dict[str, Any]
    ) -> float:
        ...
        if "seniority_indicators" in strategy:
            ...
            "headline_analysis" in strategy
            and "seniority_keywords" in strategy["headline_analysis"]
        ):
            seniority_indicators = [
                s.lower() for s in strategy["headline_analysis"]["seniority_keywords"]
            ]
        ...
        for indicator in seniority_indicators:
            if indicator in analysis_text:
                ...
                if any(
                    exec_word in indicator
                    for exec_word in ["cto", "ceo", "vp", "director", "head of"]
                ):
                    bonus = max(bonus, 4.0)  # Executive level
                elif any(
                    prin_word in indicator
                    for prin_word in ["principal", "staff", "architect"]
                ):
    ...
    def _calculate_tech_stack_bonus(
        self, analysis_text: str, strategy: Dict[str, Any]
    ) -> float:
        ...
        if "key_technologies" in strategy:
            ...
            "headline_analysis" in strategy
            and "tech_stack_signals" in strategy["headline_analysis"]
        ):
            key_technologies = [
                t.lower() for t in strategy["headline_analysis"]["tech_stack_signals"]
            ]
...
def save_outreach_results(
    outreach_data: Dict[str, Any], output_file: str = None
) -> str:
    """
