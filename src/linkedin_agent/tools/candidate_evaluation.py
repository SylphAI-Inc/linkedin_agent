...
def evaluate_candidates_quality(
    min_quality_threshold: Optional[float] = None, target_count: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    log_info(
        f"   Quality threshold: {min_quality_threshold}, Target count: {target_count}",
        phase="EVALUATION",
    )
    ...
    log_debug(
        f"Retrieved candidates length: {len(candidates) if isinstance(candidates, list) else 'N/A'}",
        phase="EVALUATION",
    )
    if candidates and isinstance(candidates, list):
        log_debug(
            f"First candidate keys: {list(candidates[0].keys()) if candidates[0] else 'Empty'}",
            phase="EVALUATION",
        )
    ...
    if not candidates:
        log_error(
            "No candidates found in global state for evaluation", phase="EVALUATION"
        )
        log_debug("Checking global workflow state...", phase="EVALUATION")

        workflow_status = get_workflow_summary()
        log_debug(
            f"Current workflow phase: {workflow_status.get('current_phase')}",
            phase="EVALUATION",
        )
        log_debug(
            f"Candidates extracted: {workflow_status.get('candidates_extracted')}",
            phase="EVALUATION",
        )
        log_debug(
            f"Raw extraction results count: {len(_workflow_state._extraction_results)}",
            phase="EVALUATION",
        )
        ...
        if extraction_results:
            log_debug(
                "Extraction results exist but appear malformed:", phase="EVALUATION"
            )
            log_debug(
                f"First extraction result type: {type(extraction_results[0])}",
                phase="EVALUATION",
            )
            log_debug(
                f"First extraction result keys: {list(extraction_results[0].keys()) if isinstance(extraction_results[0], dict) else 'Not dict'}",
                phase="EVALUATION",
            )
        ...
        return {
            ...
            "debug_info": {
                ...
                "candidates_extracted_count": workflow_status.get(
    ...
    for i, candidate in enumerate(candidates, 1):
        ...
        evaluation_result["meets_threshold"] = (
            llm_assessment.overall_score >= min_quality_threshold
        )
    ...
    avg_score = (
        sum(quality_stats["scores"]) / len(quality_stats["scores"])
    ...
    log_info(
        f"   Above Threshold: {quality_stats['above_threshold']}/{quality_stats['total_evaluated']}"
    )
    ...
    evaluation_result_data = {
        ...
        "quality_candidates": [
            ...
            if c["overall_score"] >= min_quality_threshold
        ],
...
def _convert_llm_assessment_to_result(
    llm_assessment: CandidateAssessment, candidate: Dict[str, Any]
) -> Dict[str, Any]:
    """Convert LLM assessment to evaluation result format"""
    ...
    return {
        ...
        "meets_threshold": llm_assessment.overall_score
        >= 7.0,  # Will be updated with actual threshold
...
def _generate_fallback_recommendation(
    stats: Dict[str, Any], quality_sufficient: bool, target_count: int, threshold: float
) -> Dict[str, Any]:
    """Generate intelligent fallback recommendations"""
    log_info("   🤔 Analyzing quality distribution and generating recommendations...")
    ...
    if above_threshold == 0:
        ...
        if (
            remaining_heap_size >= candidates_needed
        ):  # Heap has enough to potentially meet target
            ...
            return {
                ...
                "params": {
                    "backup_offset": processed_count,
                    "backup_limit": min(remaining_heap_size, 6),
                },
            }
        ...
        if remaining_heap_size >= candidates_still_needed:  # Heap likely has enough
            ...
            return {
                ...
                "params": {
                    "backup_offset": processed_count,
                    "backup_limit": min(
                        remaining_heap_size, candidates_still_needed + 2
                    ),
                },
            }
